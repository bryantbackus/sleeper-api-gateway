...
async function displaySummary(config) {
  ...
  console.log('\n🔗 Service URLs:')
  console.log(`   API: http://${config.domain || 'localhost'}/`)
  console.log(`   Health Check: http://${config.domain || 'localhost'}/health`)
  console.log(`   API Documentation: http://${config.domain || 'localhost'}/`)
...
async function main() {
  ...
  const startSuccess = await startDockerServices()
  if (!startSuccess) {
